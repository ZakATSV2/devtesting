...
do
	...
	function syde:Load(Config)
		...
		local statusColors = {
			...
			Detected = { Color = Color3.fromRGB(229, 44, 47), Text = '<font color="#e52c2f">Detected/Fully Risky</font>' },
				...
				local success, err = pcall(function()
					if isfile and not isfile(fullPath) then
						...
						if not content:find("Protected UI") then
							warn("[SYDE] UI integrity failed — possible tampering detected.")
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				if shouldPrompt then
					if request then
						...
						warn("[SydeUI] Request function not available — cannot send Discord invite.")
					end
...
function syde:Init(library)
	...
	do
        ...
        element:Paragraph({
			Title = 'Status',
			Content = 'Your Up To Date! (Stable R3.2)'
